...
class ResPartner(models.Model):
    ...
    def _compute_displayed_image(self):
        ...
        if self.simple_pdf_test_file:
            ...
            _logger.warning("\n\n Image changed self.displayed_image = False")
            self.displayed_image = False
    ...
    def pdf_simple_test_run(self):
        ...
        if partner_ok:
            ...
            parsed_inv = self.simple_pdf_parse_invoice(
                file_data, test_info, raw_text_dict)
            key2label = {
    ...
    @api.model
    def simple_pdf_parse_invoice(self, file_data, test_info=None, val_dict=False):
        _logger.info("Overriding: simple_pdf_parse_invoice")
        ...
        if not val_dict:
            raw_text_dict = aiio.simple_pdf_text_extraction(
                file_data, test_info)

        raw_text_dict = val_dict
        ...
        for field in partner.simple_pdf_field_ids:
            _logger.debug("Working on field %s", field.name)
            try:
        ...
        _logger.info("simple pdf parsed_inv=%s", parsed_inv)
        return parsed_inv
...
#                 self.displayed_image = aiio._get_displayed_image(
#                     file_data=file_data, monochrome_threshold=self.monochrome_threshold)
#                 # _logger.warning(f"{self.displayed_image}")
...
#         partner_id = self.simple_pdf_match_partner(
#             raw_text_dict["all_no_space"])
#         if not partner_id:
#             parsed_inv = {"chatter_msg": [
#                 "Simple PDF Import: count not find Vendor."]}
