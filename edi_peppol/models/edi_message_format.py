...
class EdiMessageFormat(models.Model):
    ...
    def pack(self, record, edi_session_line):
        ...
        edi_message = self.env['edi.message'].create( {
                'consignor':record.company_id.partner_id.id, #TODO: Borde finnas en kontakt på edi.transport
                'sender':record.company_id.partner_id.id,
                'consignee':record.partner_id.id,
                'reciever':record.partner_id.id,
                })

        edi_session_line.message_id = edi_message
        edi_message.message_format_id = self.id

        if self.name == "urn:fdc:peppol.eu:poacc:trns:order:3":
            payload = self.order_pack(purchase_order)
            edi_message.payload = payload
            edi_message.payload_filename = f"peppol_outgoing_{purchase_order.name}.xml"
            edi_message.name = f'Message {edi_message.id}'
            return edi_message
        elif self.name == "urn:fdc:peppol.eu:poacc:trns:order_response:3":
            return self.order_response_pack(purchase_order)
        else:
            return super().pack()

        return edi_message

    def unpack(self, peppol_file, peppol_file_name):
        if self.name != "urn:fdc:peppol.eu:poacc:trns:order:3":
            return super(EdiMessageFormat, self).pack()
        ...
        try:
            if self.env.company.company_registry != data_dict['Order']['cac:SellerSupplierParty']['cac:Party']['cac:PartyIdentification']['cbc:ID']['#text']:
                # FIXME: In the future, dont stop the process and instead flag it
    ...
    def order_pack(self, record):
        purchase_order = record

        xml_template = tools.misc.file_path("edi_peppol/static/src/xml/peppol_order_template.xml")

        _logger.info('edi.message.format.order_pack()')
    ...
    def order_response_pack(self, record):
        pass