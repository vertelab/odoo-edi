    ...
    def _message_set_main_attachment_id(self, attachment_ids):  # todo move this out of mail.thread
        ...
        if self._name == 'account.move' and self._message_set_main_attachment_id:
            try:
                ...
                existing_inv = invoice_import_id.invoice_already_exists(parsed_info["partner"]["recordset"], parsed_info)
                #_logger.warning(f"{existing_inv=}")
                if existing_inv:
                    message = _(
                    ...
                    #TODO SKRIV EN KOMMENTAR PÅ DUBBLETTEN
                    # ~ self.env['account.journal'].write_message_with_attachment(self, message,
                    # ~ self.message_main_attachment_id.name, self.message_main_attachment_id.datas)
                    ...
                    _logger.warning(f"{partner_id=}, {company_id=}")
                    aiico = self.env["account.invoice.import.config"]
                    import_configs = aiico.search(
                        [("partner_id", "=", partner_id.id), ("company_id", "=", company_id)]
                    )
                    ...
                    invoice_import_id.import_config_id = import_configs
                    if self.partner_id == False:
                        self.partner_id = partner_id
                    
                    invoice_import_id.update_invoice()
                    # ~ invoice_import_id.post_process_invoice()
...
class AccountJournal(models.Model):
    ...
    def _create_invoice_from_attachment(self, attachment_ids=None):
        # ~ _logger.warning("_create_invoice_from_attachment"*100)
        ...
        for attachment in attachments:
            ...
            try:
                ...
                if existing_inv:
                    ...
                    existing_invoice.to_check=True
                    # ~ existing_invoice.message_post(body=message)
                else:
