    ...
    def pack(self):
        if self.edi_type.id == self.env.ref("edi_af_aisf_trask.asok_contact").id:
            if not self.model_record or self.model_record._name != "res.partner":
                raise Warning(
                    "Attached record is not a res.partner or not a jobseeker! {model}".format(
            ...
            body_dict["base_url"] = self.edi_type.type_mapping.format(
                path="ais-f-arbetssokande/v2/kontakt/{sokande_id}".format(
                    sokande_id=obj.customer_id
                )
            )
            ...
            if obj.last_contact_type and obj.last_contact_app:
                data_dict[
                    "senasteKontaktTyp"
                ] = obj.last_contact_type_app  # Possible values: B, T, E, P, I
                data_dict["senasteKontaktDatum"] = obj.last_contact_app.date().strftime(
                    "%Y-%m-%d"
                )  # "2019-10-02",
            if obj.next_contact_type and obj.next_contact_app:
                data_dict["nastaKontaktTyper"] = [
                    "%s" % obj.last_contact_type_app
                ]  # Possible values: B, T, E, P, I
                data_dict["nastaKontaktDatum"] = obj.next_contact_app.date().strftime(
                    "%Y-%m-%d"
                )  # "2019-12-31",
                data_dict["nastaKontaktTid"] = obj.next_contact_app.time().strftime(
                    "%H:%M"
                )  # "11:30",
