...
class ica_label_create(wizard.interface):
    ...
    def _create(self, cr, uid, data, context):
        ...
        for jid in data['ids']:  # Om det är flera markerade dagjournaler
            ...
            package_cr = pooler.get_pool(cr.dbname).get('product.packaging')
            ...
            if order_ids:
                for order_record in order_cr.read(cr, uid, order_ids):
                    ...
                    for line in line_cr.read(cr, uid, line_cr.search(cr, uid, [("order_id","=",order_record['id'])])):
                        ...
                        typeofpackage = package['ul'][1] # Fel är id istf name

                        netsvc.Logger().notifyChannel("create_label", netsvc.LOG_INFO,"Packages '%s' '%s' TypeofPackage %s." % (product['packaging'],package['ul'],typeofpackage))

     
                        q=1
                        while q<=line['product_uom_qty']:
                            q+=1
                            edi_record = edi_cr.read(cr, uid, 1, )
                            if not edi_record['next_sscc']:
                                edi_record['next_sscc']=1
                                edi_record['next_desadv']=1
                                res = edi_cr.create(cr, uid, {'next_sscc': edi_record['next_sscc'],  'next_desadv': edi_record['next_desadv'] })
                            ...
                            label = label_cr.create(cr, uid, {
                                    ...
                                    'deldateshop':      datetime.datetime.strptime(order_record['date_requested'],'%Y-%m-%d %H:%M:%S').strftime('%Y%m%d'),
