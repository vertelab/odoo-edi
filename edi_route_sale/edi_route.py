...
class sale_order(models.Model):
    ...
    def create(self, vals):
        ...
        if order:
            order.route_id.edi_action('sale_order.create',order=order)
        return order
    @api.multi
    def action_cancel(self):
        ...
        for order in self:
            if order.route_id:
                order.route_id.edi_action('sale_order.action_cancel',order=order,res=res)
        return res
    @api.multi
    def action_button_confirm(self):
        ...
        for order in self:
            if order.route_id:
                order.route_id.edi_action('sale_order.action_button_confirm',order=order,res=res)
        return res
    @api.multi
    def action_wait(self):
        ...
        for order in self:
            if order.route_id:
                order.route_id.edi_action('sale_order.action_wait',order=order,res=res)
        return res
    @api.multi
    def action_done(self):
        ...
        for order in self:
            if order.route_id:
                order.route_id.edi_action('sale_order.action_done',order=order,res=res)
        return res
    @api.multi
    def action_ship_create(self):
        ...
        for order in self:
            if order.route_id:
                order.route_id.edi_action('sale_order.action_ship_create',order=order,res=res)
        return res
    @api.multi
    def action_invoice_create(self, grouped=False, states=None, date_invoice = False):
        ...
        if len(invoices)>0:
            if self.route_id:
                self.route_id.edi_action('sale_order.action_invoice_create',order=self[0],invoice=invoices[-1])
        return res
    @api.multi
    def action_invoice_cancel(self):
        ...
        for order in self:
            if order.route_id:
                order.route_id.edi_action('sale_order.action_invoice_cancel',order=order,res=res)
        return res
    @api.multi
    def action_invoice_end(self):
        ...
        for order in self:
            if order.route_id:
                order.route_id.edi_action('sale_order.action_invoice_end',order=order,res=res)
        return res
    @api.multi
    def action_ignore_delivery_exception(self):
        ...
        for order in self:
            if order.route_id:
                order.route_id.edi_action('sale_order.action_ignore_delivery_exception',order=order,res=res)
        return res
