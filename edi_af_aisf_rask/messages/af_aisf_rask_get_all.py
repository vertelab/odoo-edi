    ...
    def unpack(self):
        if self.edi_type.id == self.env.ref("edi_af_aisf_rask.rask_get_all").id:
            ...
            if body.get("processStatus", {}).get("skyddadePersonUppgifter"):
                _logger.info(
                    "RASK-SYNC - jobseeker with id: % has protected information. Delete it."
                    % (obj.id)
                )
                if obj.lastname == "new object":
                    obj.unlink()
                    self.res_id = False
            ...
            if office_code:
                office_obj = self.env["hr.department"].search(
                    [("office_code", "=", office_code)]
                )
            else:
                office_obj = False
            sun_obj = self.env["res.sun"].search(
                [("code", "=", body.get("utbildning", {}).get("sunKod"))]
            )
            if not sun_obj:
            ...
            skat_obj = self.env["res.partner.skat"].search(
                [("code", "=", body.get("kontakt", {}).get("sokandekategoriKod"))]
            )
            if skat_obj:
                skat_obj = skat_obj.id

            education_level_obj = self.env[
                "res.partner.education.education_level"
            ].search([("name", "=", body.get("utbildning", {}).get("utbildningsniva"))])
            ...
            users_obj = self.env["res.users"].search(
                [
                    (
                        "login",
                        "=",
                        body.get("kontor", {}).get("ansvarigHandlaggareSignatur"),
                    )
                ]
            )
            if users_obj:
            ...
            nasta_kontakttyper_list = body.get("kontakt", {}).get(
                "nastaKontakttyper", {}
            )
            ...
            jobseeker_dict = {
                ...
                "customer_since": body.get("processStatus", {}).get(
                    "aktuellSedanDatum"
                ),
                "share_info_with_employers": body.get("medgivande", {}).get(
                ...
                "deactualization_date": body.get("processStatus", {}).get(
                    "avaktualiseringsDatum"
                ),
                "deactualization_reason": body.get("processStatus", {}).get(
            ...
            if obj:
                obj.write(jobseeker_dict)
            else:
                obj = self.env["res.partner"].create(jobseeker_dict)
            ...
            if sun_obj:
                obj.education_ids = [
                    (
                        ...
                        0,
                        [
                            self.env["res.partner.education"]
                            .create(
                                {
                                    "sun_id": sun_obj.id,
                ...
                if streetaddress:
                    ...
                    if country_name:
                        country_obj = (
                            ...
                            .search([("name", "=", country_name)])
                        )
                        if country_obj:
                            country_obj = country_obj.id

                    if address.get("adressTyp") == "FBF":
                        obj.address_co = co_address
                        obj.street = street
                        obj.street2 = street2
                        obj.zip = zip
                        obj.city = city
                        obj.country_id = country_obj
                    elif (
            ...
            # should be removed
            self._cr.commit()
            log_msg = "Message processed for res.partner: %s" % obj.id if obj else ''
